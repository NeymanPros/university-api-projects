...
use crate::{ChatProgress, CorrFilms, FilmNames, MainTable, Next};
...
pub async fn get_pearson(bot: Bot, chat_id: ChatId, chat_progress: ChatProgress, corr_films: CorrFilms, main_table: MainTable, film_names: FilmNames) -> ResponseResult<()> {
    let answer = request_pearson(chat_id, chat_progress, corr_films, main_table).await?;
    ...
    let bot_write = async |ans: Vec<(u16, f32)>, send: String| {
...
async fn request_pearson(chat_id: ChatId, chat_progress: ChatProgress, corr_films: CorrFilms, main_table: MainTable) -> ResponseResult<Option<Vec<(u16, f32)>>> {
    let user_scores = {
    ...
    let mut pearson = crate::storage::predict_pearson(corr_films.clone(), &user_scores, &main_table, k, 0.0).await;
    if pearson.len() <= 3 {
        pearson = crate::storage::predict_pearson(corr_films.clone(), &user_scores, &main_table, k * 2, 0.0).await;
    }
    if pearson.len() <= 3 {
        pearson = crate::storage::predict_pearson(corr_films, &user_scores, &main_table, k * 4, 0.0).await;
    }
