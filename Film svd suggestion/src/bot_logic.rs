...
use crate::{get_unlocked, ChatProgress, FilmNames, Model, Next};

pub async fn handle_message(bot: Bot, msg: Message, chat_progress: ChatProgress, film_names: FilmNames, model: Arc<Model>) -> ResponseResult<()> {
            ...
            Next::None {} => {
                match text {
                    "/start" => {
                        ...
                        /get_recommendations - if you added at least 1 film + score.").await?
                    }
                    "/add_film" => {
                        enter_add_film(bot, msg.chat.id, chat_progress).await?
                    }
                    "/get_recommendation" => {
                        get_sdv(bot, msg.chat.id, chat_progress, film_names, model).await?;
                    }
                    "/my_scores" => {
